...
"#topo-map{flex:1;width:100%}\n"
/* z-index bumped well above Leaflet's own panes (tiles/overlay/marker
 * panes run up to ~650, controls to 1000) now that this floats over a
 * real Leaflet map instead of a plain canvas -- the old z-index:4/5
 * would otherwise render underneath every map layer. */
"#topo-empty{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);max-width:480px;text-align:center;pointer-events:none;z-index:1000}\n"
"#topo-legend{position:absolute;left:10px;top:10px;color:#64748b;font-size:11px;background:rgba(15,23,42,0.85);padding:7px 11px;border-radius:3px;border:1px solid #334155;z-index:1000;pointer-events:none}\n"
...
"  <div id=\"topo-legend\"><span class=l-node></span> node, size = frames seen &nbsp;|&nbsp; <span class=l-repeater></span> repeater/room/sensor &nbsp;|&nbsp; <span class=l-edge></span> observed RX, color = SNR &nbsp;|&nbsp; <span class=l-relay></span> relay path, width = exchange count &nbsp;|&nbsp; click a node to inspect<span id=\"topo-relay-stats\"></span><span id=\"topo-geo-badge\"></span></div>\n"
"  <div id=\"topo-map\"></div>\n"
"</div>\n"
...
"let tileLayer=null, topoTileLayer=null;\n"
/* Drives both the Live map's tile layer AND the Topology tab's own
 * Leaflet map (topoMap, below) -- two separate Leaflet instances, so
 * two separate tile layers, kept in the same theme lock-step. */
"function setTheme(name){const html=document.documentElement;if(name==='light')html.classList.add('light');else html.classList.remove('light');try{localStorage.setItem('theme',name);}catch(e){}const url=(name==='light')?'https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png':'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png';if(tileLayer)map.removeLayer(tileLayer);tileLayer=L.tileLayer(url,{maxZoom:19,attribution:'(c) OSM (c) CARTO'}).addTo(map);if(topoTileLayer)topoMap.removeLayer(topoTileLayer);topoTileLayer=L.tileLayer(url,{maxZoom:19,attribution:'(c) OSM (c) CARTO'}).addTo(topoMap);const btn=document.getElementById('theme-toggle');if(btn)btn.textContent=(name==='light')?'light':'dark';}\n"
...
"const topoMap = L.map('topo-map').setView([39.5, -98.0], 4);\n"
...
"// =============================================================\n"
"// Topology tab: neighbor graph rendered on its own Leaflet map\n"
"// (topoMap, see its setup near the main map above). A node only\n"
...
"// layout state: Leaflet reads live positions straight from\n"
"// markers{}/stationMarker via topoGeoLatLng() below on every render.\n"
...
"// Running tallies surfaced in the Topology legend (see topoMapRender) so\n"
...
"  if (!topoNodes[id]) topoNodes[id] = {id};\n"
"  return topoNodes[id];\n"
"}\n"
/* topoGeoLatLng -- real-world position for a topology node. For a
 * normal node, the same Leaflet marker position (placeMarker()) the
 * Live map already plots -- null if unknown, most nodes discovered
 * via CONTROL/GRP_TXT/envelope-only hashes never get one.
 *
...
"  if (id !== RX_STATION_ID) {\n"
"    const m = markers[id];\n"
"    return (m && m.getLatLng) ? m.getLatLng() : null;\n"
"  }\n"
"  if (stationMarker) return stationMarker.getLatLng();\n"
"  let sumLat = 0, sumLng = 0, sumW = 0;\n"
"  for (const k in topoEdges) {\n"
"    const e = topoEdges[k];\n"
"    if (e.kind !== 'relay') continue;\n"
"    const otherId = e.a === RX_STATION_ID ? e.b : (e.b === RX_STATION_ID ? e.a : null);\n"
"    if (!otherId) continue;\n"
"    if (nodeKind(nodes[otherId]) === 'people') continue;\n"
"    const mk = markers[otherId];\n"
"    if (!mk || !mk.getLatLng) continue;\n"
"    const ll = mk.getLatLng();\n"
"    const w = Math.min(1, 0.2 + Math.log2(1 + (e.count||1)) * 0.25);\n"
"    sumLat += ll.lat * w; sumLng += ll.lng * w; sumW += w;\n"
"  }\n"
"  return sumW ? L.latLng(sumLat/sumW, sumLng/sumW) : null;\n"
"}\n"
...
"function topoMapRenderScheduled(){\n"
"  if (topoMapRafQueued) return;\n"
"  topoMapRafQueued = true;\n"
"  requestAnimationFrame(()=>{ topoMapRafQueued = false; topoMapRender(); });\n"
"}\n"
/* topoMapRender -- full clear + redraw of both layer groups from the
 * current topoEdges/topoNodes state. A full redraw (rather than
 * incrementally diffing markers/lines in place) is simpler and cheap
 * enough at the sizes this tracks (TOPO_NODE_MAX=200) -- no separate
 * class of stale-marker bugs to worry about. Edge line weight scales
...
"      : '';\n"
"  }\n"
"  const totalEdges = Object.keys(topoEdges).length;\n"
/* Positioned-node/edge count is the honest headline here: unlike the
 * old canvas force layout (which could always draw SOMETHING for
 * every known node), this view can only show what has real
 * coordinates -- surfacing the fraction shown vs. tracked makes that
 * limitation visible instead of silently dropping nodes/edges. */
"  const geoBadgeEl = document.getElementById('topo-geo-badge');\n"
"  if (geoBadgeEl) {\n"
"    geoBadgeEl.textContent = totalEdges\n"
"      ? ` \\u00b7 ${nodesShown} node(s), ${edgesShown}/${totalEdges} edge(s) shown (needs both ends positioned)`\n"
"      : '';\n"
"  }\n"
"  if (!totalEdges) {\n"
"    topoEmpty.style.display = 'block';\n"
"    topoEmpty.textContent = 'Waiting for the first frame... nodes appear as soon as the sniffer hears them, and link up via NEIGHBORINFO and relay-hop hints.';\n"
"  } else if (!edgesShown) {\n"
"    topoEmpty.style.display = 'block';\n"
"    topoEmpty.textContent = `${totalEdges} link(s) observed, but no node has a known position yet -- positions come from the same ADVERT/position data the Live map uses.`;\n"
"  } else {\n"
